...
Canon as of **D191** (2026-09-10). One page of current truth. When a new
...
- **Converged every pass**: campaign sends/day (warmups excluded,
  ...
  `message_per_day` (D24). A `tenant_rate_limit` incident holds those
  Outlook / Microsoft senders at **0**/day until **15 minutes after
  the next UTC midnight** (Microsoft's cap reset), then D183
  restores 15. Gap-enforce, fan-out, top-up, and campaign-check
  must not write 15 over an active hold (D191). Gmail/SMTP on the
  same campaign keep their standing cap; campaign status does not
  change (D148). 10-minute minimum gap (D30/D35) — held at
...
- **Fan-out**: a client-owned inbox belongs on every ACTIVE campaign for its
  ...
  Insight. Client-rest does not strip that exclusive set (D189).
  BCP-owned domains
...
- **Rest (pods)**: each client's inboxes split into a stable, even A/B
  ...
  on a paused/stopped campaign that is not sending (D169). **Exception
  (D189):** client-rest does not unlink or bench mailboxes off Insight
  campaigns for client 345263 (the named D184 ids, or any campaign
  whose name starts with `Insight `). D184 exclusive Insight staff
  survives the fortnight A/B cycle; Engagers / other SalesGlider
