...
Canon as of **D202** (2026-09-22). One page of current truth. When a new
...
- **Converged every pass**: campaign sends/day (warmups excluded,
  ...
  one-line, extra-line, or **pool / POC leftover** foreign-client
  signature is a `mailbox_sig` finding and is **written on that
  check pass** (D74/D125) — never left waiting for the 6-hour
  converge. A **two-line known-named-client signature** (Parlay on
  a TechEvo scan, BCP on a Parlay scan) is **not** restamped onto
  the campaign's brand — peel the foreign membership (D26/D202).
  Chat never `updateEmailAccount` signatures (D202). **Copy that contains `Insight`**
...
- **One client per sender, hard.** An inbox sits on every ACTIVE campaign of
  ...
  **Exclusive-attach / no-multi-link is pool generics only (D200).**
  ...
  Goliath while they sit on that client's campaigns. They are also
  not restamped onto a *different* named client's brand while
  attached (D202) — peel the leak, keep the two-line.
...
Never spend, purge, bypass warmup/holds, or rewrite mailbox
signatures from chat (D18/D202).
