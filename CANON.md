...
- **Converged every pass**: 30 campaign sends/day (warmups excluded, D24),
  ...
  waiting for the 6-hour converge. **Copy that contains `Insight` never
  gets auto signature append** (D177): if sequence `email_body` /
  variants include the exact substring `Insight` (capital I), D92 does
  not write `%signature%` / `{{Signature}}`, and the same pass **strips**
  those placeholders if present. `insight` / `INSIGHT` alone do not
  match. Other clients' copy without that substring still gets D92.
