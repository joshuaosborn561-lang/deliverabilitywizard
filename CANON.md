...
Canon as of **D158** (2026-09-02). One page of current truth. When a new
...
- 80% same-ESP live / 85% launch (promo tab = miss) are **readings** — the
  ...
  sender-domain path). That is not a Slack page (D71/D158).
...
- A campaign in spam is a flag, not a verdict (D49). The flag is raised by
  reply-collapse (D69) **or** by same-ESP inbox under 80% on that campaign's
  canary-copy test or live placement test (D158) — shells stay off the live
  board (D126), but canary-copy ugliness for an ACTIVE live campaign counts.
