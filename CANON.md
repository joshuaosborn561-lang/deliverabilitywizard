...
Canon as of **D204** (2026-09-23). One page of current truth. When a new
...
- **One client per sender, hard.** An inbox sits on every ACTIVE campaign of
  ...
  *staffable* senders (D197/D198/D199/D200). Raw membership
  (disconnected / resting / canary leftovers) is not surplus.
  **Exclusive-attach / no-multi-link is pool generics only (D200).**
  A named client seat (e.g. techevolution* tagged to TechEvo
  521881) **may** sit on every ACTIVE campaign of that same
  client. Do not peel it for "multi-link" / exclusive-attach.
  Still peel: (1) pool / generic seats multi-linked across
  campaigns, even the same client; (2) a seat whose
  `client_id` / tag is a foreign client on this campaign;
  (3) rotating undedicated pool shares that break exclusivity.
  The ≥40 staffable floor still gates every peel (D199).
...
- **Floor = max(that client's named on-week pod, 40 per POD)** at
  **client inventory** (D58, D82, D196, D197, D198, D199, D200, D203).
  ...
  carve-outs stay as documented (D81/D82/D204) — do not invent new POC
  ...
  **named seats**, not free-pool generics: fan-out across every
  ...
  free-pool restaff onto PowerGryd. Leave-alone: no START / PAUSE
  / Retire / teardown of live PowerGryd by automation — Josh
  cancels the client API when the POC ends.
...
- **Rest (pods)**: each client's **named** senders split into a
  **static** even A/B (D43/D203) that is **ESP-balanced ~50/50 within Outlook and within Gmail**
  ...
  ACTIVE rest is unchanged. **Exception (D204):** PowerGryd's 40
  dedicated seats have **no A/B rest and no pods** — they stay on
  every PowerGryd campaign. Warmup stays
...
- **Generics** staff only a POC client (currently Goliath;
  PowerGryd 592842 is also a POC but takes **no** rotating-pool
  attaches — its 40 dedicated seats are the staff, D204)
  (D81/D82/D193/D204) for *new rotating-pool attaches* **beyond** a
  ...
  **Dedicated generics
  ...
  Do **not** rotate the same *pool generic* id across clients or
  camps (exclusive-attach). Named client seats are not exclusive —
  they fan out across that client's campaigns (D26/D84/D200).
  Leftover D134 Slack /
  ...
  carve-outs stay as documented (D81/D82/D204).
  **Exception (D197/D198/D199/D200/D203):** dedicated named-client generic
  ...
  generic-rest, client-rest, or Insight shared-staff unlink as
  foreign Goliath, even above 40. `pod-cover` does not unlink live
  ACTIVE seats. Surplus *undedicated* rotating-pool seats above 40
  *staffable* may still come off. A dedicated **pool** seat linked
  to a second client's campaign still peels that foreign membership
  (floor-gated), and a pool generic multi-linked across two camps
  of the *same* client still peels the extra link (D200). Named
  same-client seats do not. An ACTIVE on-week campaign under 40 staffable is
  an `understaffed` finding. The **staffable peel** floor is the
  same count `/health` uses — disconnected leftovers must not look
  like surplus.
  ...
  **Exception (D204):** the 40 PowerGryd dedicated mailbox ids do
  not count toward that census — a pool host they sit on stays
  plan-generic (or stays with its other real client) and is never
  used to re-point those seats off 592842.
...
Oct 15 hold, Insight SEG pause, or PowerGryd POC leave-alone (D204).
