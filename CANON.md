...
- Disconnected mailboxes are re-authed every health pass from the shared
  account book (D94/D132); reconnect results Slack as action results. A
  mailbox-list 5xx with no accepted book pages once as `ops_alert` (D149),
  never as a fake 0-disconnect wave.
...
episode, recovery noted), a wrong deploy identity at boot, and Smartlead's
mailbox-list API out with no accepted book. Alerts and
