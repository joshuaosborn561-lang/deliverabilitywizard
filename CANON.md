...
- **Campaign client tag** (D77/D85/D144): health writes Smartlead
  `client_id` from a unique name match, including short brand acronyms
  (MSRS). Restored Nieto / MSRS / Positive campaigns get a client row if
  Smartlead no longer has one, then the tag. Names with no unique match
  stay on the EOD brief until a human tags them.
