...
- 80% same-ESP live / 85% launch (promo tab = miss) are **readings** — the
  ...
  **re-queues** (D164). The isolation-branch sweep re-evaluates those
  ...
  reading. First detect / mark-suspect and each isolation
...
- A campaign in spam is a flag, not a verdict (D49). The flag is raised by
  ...
  **copy** path on `content_block` + ugly canary unless known-good also
  ...
  - Latest run **INCONCLUSIVE** (or `evaluatedAt` set but the latest
    ...
    **re-queue and re-read** on the next 15-minute isolation-branch
    pass (D164), including PAUSED lives already flagged. Do not treat
    an old COPY stamp — or `evaluatedAt` alone — as a lock. Missing
    copy-canary `testId` is backfilled; without it the unwarmed
    reading stays null and the verdict cannot leave INCONCLUSIVE.
