...
Canon as of **D177** (2026-09-08). One page of current truth. When a new
...
- **Converged every pass**: 30 campaign sends/day (warmups excluded, D24),
  ...
  waiting for the 6-hour converge. **Insight campaigns never get auto
  signature append** (D177): a name starting with `Insight`
  (case-insensitive), or a campaign tagged to an Insight client, is not
  a `missing_signature_tag` and D92 does not write `%signature%` /
  `{{Signature}}` onto its sequences — even when it is wrongly tagged
  SalesGlider. BCP, Goliath, Parlay, TechEvo, and SalesGlider still get
  D92.
...
`First Last / {Client name}` (D92) except Insight campaigns (D177).
