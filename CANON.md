...
Canon as of **D199** (2026-09-21). One page of current truth. When a new
...
| Campaign check | Hourly (yields to a running health pass, D122) | Re-inspect blocked first-checks; sweep pod/shell posture, signatures, client tag, one-client, canary coverage (both kinds), staffing floor (D81/D82/D196/D197/D198/D199 — on-week client pod, never below 40 staffable), **step-2 delay** (D186 — `seq_delay_details.delay_in_days` must be 2 on the second email; auto-fix via `sequencesForWrite`; canary / pod-control / word-hunt shells and 1-step instrumentation skipped), **merge-tag fill on ACTIVE campaigns that use custom `{{tags}}`** (D180 — multi-offset lead sample + cheap sent-body hole check; pages `merge_tag_blank`, never edits copy). Reads the shared account book, never its own fetch (D132). |
...
- **One client per sender, hard.** An inbox sits on every ACTIVE campaign of
  ...
  *staffable* senders (D197/D198/D199). Raw membership
...
- **Floor = max(that client's on-week pod, 40)** (eligible inboxes in
  this fortnight's A or B cohort — D58, D82, D196, D197, D198, D199).
  Peels consult the **staffable attached** count on that ACTIVE
  campaign, never raw `campaign_ids` length (D199). Same
...
- **Rest (pods)**: each client's inboxes split into a stable, even A/B
  ...
  (D197/D198/D199; surplus above 40 may still rest). **Exception
...
- **Generics** staff only a POC client (currently Goliath)
  ...
  **client-inbox only** for those new attaches. **Dedicated generics
  ...
  Do **not** rotate the same generic id across clients or camps.
  Leftover D134 Slack /
  ...
  **Exception (D197/D198/D199):** dedicated named-client generic
  ...
  *staffable* may still come off. A dedicated seat linked to a
  second client's campaign still peels that foreign membership
  (floor-gated). An ACTIVE on-week campaign under 40 staffable is
