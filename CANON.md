...
- **Converged every pass**: campaign sends/day (warmups excluded,
  ...
  on this path for mailboxes that also sit on a SalesGlider
  campaign (D31/D92 still apply there). **D184 — Insight
  campaigns only** (ids 3921647, 3921651, 3921650, 3921654,
  3921656, 3921653, 3921659; or copy containing `Insight`):
  Smartlead has no campaign-level "don't append mailbox
  signature" switch, so the campaign-scoped fix is keep
  `%signature%` out of the body **and** blank a mailbox
  signature only when that inbox is **exclusively** staffed on
  those Insight campaigns. A mailbox that also sits on any
  non-shell SalesGlider campaign keeps Name / SalesGlider —
  finding, no write. `desiredMailboxSignature` for the
  SalesGlider client is unchanged. Do not re-introduce
...
the P.S. (D178). Exclusive Insight-staffed mailboxes with a
SalesGlider footer are blanked; shared SalesGlider mailboxes are
not (D184).
