/**
 * D67 / D73 — Smartlead campaign bounce auto-pause.
 *
 * Under-1k lists are a small TAM. The fleet default (7%) trips pause on
 * ordinary variance. Josh set those campaigns to 20%. Under-1k is a name
 * match only — do not infer it from band labels like 501-1000.
 *
 * D73 — Goliath campaigns also hold 20%. Displacement M sat at 13.7%
 * bounce and the 7% auto-pause kept killing it. Company-size bands in
 * the name are not list size.
 */
...
/** True when the campaign name is a Goliath list (D73). */
...
export function desiredBounceAutopausePercent(
  ...
  if (isUnder1kCampaign(name) || isGoliathCampaign(name)) return under1kPercent;
  return null;
}
