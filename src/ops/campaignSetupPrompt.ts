...
export function campaignSetupPrompt(): string {
  return [
    ...
    "7. Placement tests are one recurring SmartDelivery schedule per campaign (every_days: 1), not a new test each morning. No plan quota (unlimited). Still ≤50 senders per test (SmartDelivery API limit). Launch bar is 85% same-ESP (promo tab = miss). 80% / bounce 5% are readings only — they do not pull. Bounce autostop (D80) pauses the campaign after 100 sends at 20%, then 7% from 500 sends; leave Smartlead bounce auto-pause off. The only automatic live pull is Josh killing a mailbox / retiring a domain; health backfills.",
    "8. Never auto-resume a campaign someone paused or stopped by hand (do not auto-START). Protective pauses we took stay in pendingResumes only. Bounce autostop pauses are not pendingResumes. A clean POC may START after signature QA.",
    "9. Do not spend, purge, or bypass warmup/holds from chat. Approvals stay on.",
    "10. After launch: first health rebuilds unproven HOLDs once, then rests clients, sits spent generics, tops up to the half-client floor, and fans out. Slack only pages a burned domain, an isolated word, the EOD scoreboard, a button result, or Allow generics.",
