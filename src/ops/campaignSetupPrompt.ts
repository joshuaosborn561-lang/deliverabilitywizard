...
export function campaignSetupPrompt(): string {
  return [
    ...
    "11. Isolation (report only): standing per-pod inbox tests answer inboxes-vs-copy. A confirmed suppressed word is a lint *warning* before launch, not a block. The wizard never edits the live sequence. Isolation-domain mailboxes never attach to a campaign. Tests are unlimited — do not hold a copy teardown for seed approval.",
  ].join("\n");
}
