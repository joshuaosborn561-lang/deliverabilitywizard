...
export function campaignSetupPrompt(): string {
  return [
    ...
    "5. 21 days from InboxKit import is the warmed-vs-unwarmed clock. Pool supply for the 50 floor is warmed only. Each ACTIVE campaign also keeps the dedicated D54 canary fleet (2 domains × 3 inboxes, Google + Outlook, warmup off) sending the campaign sequence so isolation can tell copy from inboxes. Pre-warmed fleets skip that wait.",
    ...
    "11. Isolation: standing per-pod inbox tests answer inboxes-vs-copy. A campaign in spam is a flag, not a domain death sentence. A confirmed suppressed word is a lint *warning* before launch, not a block. The wizard never edits the live sequence until Josh or Cayden tap Switch the word. Retiring a domain, buying replacements, or buying the canary fleet waits for Josh. Isolation-domain mailboxes never attach to a campaign. Tests are unlimited — do not hold a copy teardown for seed approval.",
  ].join("\n");
}
