...
export function campaignSetupPrompt(): string {
  return [
    ...
    "6. Every mailbox: 30 campaign emails/day (warmups not included), 10-minute gap, warmup ON (except the D54 canary fleet), plain Name / Brand signature that matches the campaign client — never another client's brand.",
    ...
    "8. Never auto-resume a campaign someone paused or stopped by hand (do not auto-START). Protective pauses we took stay in pendingResumes only. Bounce autostop pauses are not pendingResumes.",
