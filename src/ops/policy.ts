...
export function classifyOpsMessage(
  ...
  if (
    ...
    return {
      type: "denied",
      reason:
        "The console cannot change fleet policy. The staffing floor (max(half each client's named inboxes per POD, 40 per POD) — D82/D203) and the type-aware mailbox cap (Outlook 15 / Gmail-SMTP 30) are owner decisions and require a reviewed code/config change (D82, D11/D24/D183, D203).",
    };
  }
