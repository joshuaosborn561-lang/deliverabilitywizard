...
/**
 * D67 / D73 — Hold Under-1k and Goliath campaigns at Smartlead bounce
 * auto-pause 20%. Everyone else is left alone (fleet default stays 7%).
 * Does not START paused campaigns (D40). D29's 7% investigate line is
 * unchanged.
 */
...
export class BounceAutopauseService {
  ...
  async run(opts: { dryRun?: boolean } = {}): Promise<BounceAutopauseResult> {
    ...
    console.log(
      `[bounce-autopause] scanned=${result.scanned} matched=${result.matched} updated=${result.updated} errors=${result.errors.length}`,
    );
    return result;
  }
}
