...
describe("CampaignBounceAutostopService (D80 / D88)", () => {
  it("D88: never pauses on a bounce band; still writes Smartlead autopause off", async () => {
    const paused: number[] = [];
    const started: number[] = [];
    const settings: Array<{ id: number; threshold: unknown }> = [];
    const analyticsReads: number[] = [];
    ...
    assert.deepEqual(analyticsReads, [], "retired bands must not re-read analytics");
    assert.equal(
