...
export class CampaignCheckService {
  ...
  async run(
    ...
    for (const campaign of campaigns as SmartleadCampaign[]) {
      ...
      const sigApplied = await this.autoApplySignature({
        ...
        name,
        brand,
        otherClientBrands: allBrands.filter((row) => row !== brand),
        accounts: accounts as SmartleadAccountWithCampaigns[],
        findings,
      });
      if (sigApplied) {
        findings = findings.filter((finding) => {
          if (finding.kind === "missing_signature_tag" && sigApplied.wroteTag) {
            return false;
          }
          if (finding.kind === "mailbox_sig" && sigApplied.wroteMailbox) {
            return false;
          }
          return true;
        });
        // D95 — tell Josh the first time we write a campaign's %signature%
        // tag. A leftover backfill already notified does not ping again.
        // Mailbox-only First Last / brand rewrites stay in logs (D71).
      ...
      const next: CampaignCheckRecord = {
        ...
        sigAutoWrittenAt: sigApplied?.wroteTag ? now : record.sigAutoWrittenAt,
      };
      if (kind === "first") {
  ...
  /**
   * D92 / D124 — missing `%signature%` and a mailbox that is not
   * First Last / client brand are written, not asked. Slack once per
   * campaign the first time we append the tag (D95). Mailbox-only
   * leftover rewrites stay in logs (D71 / D98).
   */
  private async autoApplySignature(input: {
    ...
    const missingTag = input.findings.some(
      (finding) => finding.kind === "missing_signature_tag",
    );
    const mailboxDrift = input.findings.some(
      (finding) => finding.kind === "mailbox_sig",
    );
    if (!missingTag && !mailboxDrift) return null;
    if (this.config.dryRun) {
      console.log(
        `[campaign-check] dry-run signature #${input.campaignId} ${input.name}`,
      );
      return { brand: input.brand, wroteTag: missingTag, wroteMailbox: mailboxDrift };
    }
    try {
      let wroteTag = false;
      if (missingTag) {
    ...
    for (const account of attached) {
      ...
      if (expected) {
        ...
        if (mismatch) {
          findings.push({
            kind: "mailbox_sig",
            detail: formatMailboxSignatureMismatch(email, mismatch),
          });
        }
      }
