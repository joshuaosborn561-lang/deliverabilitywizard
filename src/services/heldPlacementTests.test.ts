...
describe("HeldPlacementTestService", () => {
  ...
  it("does not block held or rest tests when quota is 0 (D45)", async () => {
    const state = new StateStore(
      `/tmp/held-quota0-${process.pid}-${Date.now()}.json`,
    );
    await state.load();
    state.markHeldInbox({
      accountId: 7,
    ...
    const held = new HeldPlacementTestService(
      loadConfig(config),
      smartlead as never,
      smartDelivery as never,
      slack as never,
    ...
    const heldResult = await held.run({ dryRun: false });
    assert.equal(heldResult.quotaBlocked, false);
    assert.equal(heldResult.created.length, 1);

    const rest = new HeldPlacementTestService(
      loadConfig(config),
      smartlead as never,
      smartDelivery as never,
      slack as never,
    ...
    const restResult = await rest.runResting({ dryRun: false });
    assert.equal(restResult.quotaBlocked, false);
    assert.equal(restResult.created.length, 1);
  });
});
